...
const getTLe = async () => {
  console.log("Updating ", satName, " TLE from celestrak.org");
  const url =
  ...
  dayOfYearTLE = parseFloat(data[1].substring(20, 31));
  meanMotionTLE = parseFloat(data[2].substring(52, 62));
  satrec = satellite.twoline2satrec(data[1], data[2]);
  initMap();
};
window.initMap = getTLe;
...
function getNextPass(latLon, beginTime, maxRoll) {
  ...
  let roll = (getAngleACabAB(nStEcef, nGsEcef, th) / Math.PI) * 180;
